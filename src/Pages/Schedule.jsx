...
const initialHorarios = [
  { id: 1, horaInicio: '08:00', horaFin: '10:00', aula: '101', grupo: 'A', materia: 'Matemáticas', docente: 'María González' },
  { id: 2, horaInicio: '10:00', horaFin: '12:00', aula: '203', grupo: 'B', materia: 'Historia', docente: 'Juan Pérez' },
  { id: 3, horaInicio: '14:00', horaFin: '16:00', aula: '105', grupo: 'C', materia: 'Física', docente: 'Ana Torres' },
];
...
export const Schedule = ({ user, setUser }) => {
  ...
  const [form, setForm] = useState({ horaInicio: '', horaFin: '', aula: '', grupo: '', materia: '', docente: '' });
  ...
  const openModal = (mode, horario = null) => {
    ...
    if (horario) {
      ...
      setForm({ horaInicio: '', horaFin: '', aula: '', grupo: '', materia: '', docente: '' });
    }
  };
  ...
  const validate = () => {
    const newErrors = {};
    if (!form.horaInicio) newErrors.horaInicio = 'Hora de inicio requerida';
    if (!form.horaFin) newErrors.horaFin = 'Hora de fin requerida';
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full bg-white rounded-xl border border-gray-200">
                  ...
                  <tbody>
                    {horarios.length === 0 ? (
                      <tr>
                        <td colSpan={7} className="text-center py-8 text-gray-500">No hay horarios registrados.</td>
                      </tr>
                    ) : (
                      horarios.map(h => (
                        <tr key={h.id} className="border-b border-gray-100 hover:bg-blue-50 transition-colors">
                          <td className="px-4 py-3">{h.horaInicio}</td>
                          <td className="px-4 py-3">{h.horaFin}</td>
                ...
                <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-lg relative">
                  ...
                  <form className="space-y-5">
                    <div>
                      ...
                      {modal.mode === 'detail' ? (
                        <div className="py-2 text-gray-900 font-medium flex items-center"><Clock className="w-4 h-4 mr-2 text-blue-600" /> {form.horaInicio}</div>
                      ) : (
                        <input
                          type="time"
                          name="horaInicio"
                          ...
                          className={`w-full px-3 py-2 border ${errors.horaInicio ? 'border-red-300' : 'border-gray-300'} rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500`}
                        />
                      )}
                      {errors.horaInicio && <p className="text-xs text-red-600 mt-1">{errors.horaInicio}</p>}
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Hora de Fin</label>
                      {modal.mode === 'detail' ? (
                        <div className="py-2 text-gray-900 font-medium flex items-center"><Clock className="w-4 h-4 mr-2 text-purple-600" /> {form.horaFin}</div>
                      ) : (
                        <input
                          type="time"
                          name="horaFin"
                          ...
                          className={`w-full px-3 py-2 border ${errors.horaFin ? 'border-red-300' : 'border-gray-300'} rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500`}
                        />
                      )}
                      {errors.horaFin && <p className="text-xs text-red-600 mt-1">{errors.horaFin}</p>}
                    </div>
                    <div>
